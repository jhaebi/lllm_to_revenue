...
if st.button('Execute Request'):
    if user_query:
        ...
        # Create a dictionary to hold variables for execution, including the 'df' DataFrame
        exec_globals = {'df': df}
    
        # Execute the generated AI output within the 'exec_globals' context
        exec(generated_ai_output, exec_globals)
    
        # Check if 'plt' (Matplotlib) was generated in the execution context
        if 'plt' in exec_globals:
            # Get the current figure from Matplotlib
            figure = exec_globals['plt'].gcf()
    
            # Display the Matplotlib figure as a Streamlit plot
            st.pyplot(figure)
